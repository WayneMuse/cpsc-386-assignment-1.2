...
## functions to add/subtract kills and ammo
func ammo_use():
	if player_ammo == 0:
		return
	player_ammo -= 1
...
#exiting, restarting, startmenu functions
func exitGame():
	print("Exiting...")
	get_tree().quit()
func restart():
	print("Reloading Scene %s" % get_tree().current_scene.name)
	get_tree().reload_current_scene()
func escapeToStartMenu():
	print("Loading Start Scene")
